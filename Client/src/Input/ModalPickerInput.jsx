...
export function ModalPickerInput({ onChange, name, value, label, type, index }) {
  const search = useSearch(state => state.search)
    ...
    let [selected, setSelected] = useState(val?.id !== -1 ? val : new Link(type[0]))
    ...
    return (<div >
        ...
        > <Text text={ getFromSearch(search,val.id,{name:"Aucun"}).name } /> </button>
